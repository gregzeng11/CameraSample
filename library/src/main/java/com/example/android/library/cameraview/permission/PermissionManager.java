...
public class PermissionManager {
     ...
     * @return
     */
    public static TPermissionType checkPermission(@NonNull TContextWrap contextWrap) {

        boolean storageGranted =
                ContextCompat.checkSelfPermission(contextWrap.getActivity(), TPermission.STORAGE.stringValue())
                        == PackageManager.PERMISSION_GRANTED;

        boolean cameraGranted = ContextCompat.checkSelfPermission(contextWrap.getActivity(), TPermission.CAMERA.stringValue())
                == PackageManager.PERMISSION_GRANTED;
    ...
    public static void requestPermission(@NonNull TContextWrap contextWrap, @NonNull String[] permissions) {
        if (contextWrap.getFragment() != null) {
            contextWrap.getFragment().requestPermissions(permissions, TConstants.PERMISSION_REQUEST_TAKE_PHOTO);
        } else {
            ActivityCompat.requestPermissions(contextWrap.getActivity(), permissions, TConstants.PERMISSION_REQUEST_TAKE_PHOTO);
        }
    }

    public static TPermissionType onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        if (requestCode == TConstants.PERMISSION_REQUEST_TAKE_PHOTO) {
    ...
    public static void handlePermissionsResult(Activity activity, TPermissionType type, TakePhoto.TakeResultListener listener) {
        String tip = null;
        switch (type) {
            case DENIED:
            ...
            case GRANTED:
                try {
                    listener.takePermissionGranted();
                } catch (Exception e) {
                    e.printStackTrace();
                    listener.takeFail(null, tip = activity.getResources().getString(R.string.tip_permission_camera_storage));
                }
